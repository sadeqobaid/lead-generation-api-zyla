...
from .providers import LeadProvider, ProviderLead
...
async def execute_lead_search(
    ...
    """Fetch provider data asynchronously and persist the result off the event loop.

    HTTP provider calls remain asynchronous, while the synchronous SQLAlchemy transaction is
    isolated in ``_persist_lead_search`` and dispatched with ``run_in_threadpool``. This avoids
    blocking FastAPI's event loop during tenant locking, suppression checks, ORM writes, and
    credit accounting while preserving one transaction for the persisted search.
    """
    filters = payload.filters.model_dump(exclude_none=True)
    records = await provider.search(filters)
    return await run_in_threadpool(
        _persist_lead_search,
        tenant_id,
        api_key_id,
        request_id,
        request_fingerprint,
...
def _persist_lead_search(
    ...
    payload: LeadSearchRequest,
    provider_code: str,
    records: list[ProviderLead],
) -> LeadSearchResponse:
    """Persist one search in a single synchronous transaction owned by this worker thread."""
    with SessionLocal() as db:
        try:
            ...
            def is_suppressed(record: ProviderLead) -> bool:
                candidates = [
                        ...
                        provenance=[
                            {
                                ...
                                "data_source": item.source.company.get("data_source")
                                or ("Built-in deterministic fixture records; development and demonstrations only." if item.source.data_status.upper() == "SYNTHETIC" else None),
                            }
                        ],
                    )
                )
            ...
            if credits_used:
                db.add(
                    CreditLedger(
                        ...
                        metadata_json={"request_id": request_id, "provider": provider_code},
                    )
                )
            db.add(
                UsageRecord(
